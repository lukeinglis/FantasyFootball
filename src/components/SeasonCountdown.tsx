...
export default function SeasonCountdown() {
  ...
  return (
    <div className="relative overflow-hidden border border-result/30 bg-surface p-8">
      <div className="stripe-pattern pointer-events-none absolute inset-0 opacity-60" aria-hidden />
      <div
        className="pointer-events-none absolute -top-8 left-1/2 h-24 w-48 -translate-x-1/2 bg-result/15 blur-3xl"
        aria-hidden
      />

      <div className="relative">
        <p className="font-[family-name:var(--font-heading)] text-xs font-semibold uppercase tracking-[0.3em] text-result">
        ...
        <div className="mt-6 flex items-baseline justify-center gap-2 text-center">
          ...
          <span className="text-2xl font-bold text-result/40 pb-5">:</span>
        ...
        <div className="mt-6">
          ...
          <div className="mt-1 h-1.5 w-full overflow-hidden bg-paper">
            <div
              className="h-full bg-result transition-all duration-1000"
...
function CountdownUnit({ value, label }: { value: number; label: string }) {
  return (
    <div>
      <p
        className="font-[family-name:var(--font-heading)] font-bold text-4xl text-ink sm:text-5xl"
      >
        {String(value).padStart(2, "0")}
      </p>
      <p className="mt-1 text-[10px] uppercase tracking-wider text-result/70">
        {label}
      </p>
    </div>
  );
