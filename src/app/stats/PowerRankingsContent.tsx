...
interface PowerRankingsContentProps {
  teams: LeagueStandings["teams"];
  scoreboards: Scoreboard[];
  currentWeek: number;
}

export default function PowerRankingsContent({
  teams,
  scoreboards,
  currentWeek,
}: PowerRankingsContentProps) {
  if (teams.length === 0) {
    return (
      <EmptyState
        icon={<span>📊</span>}
  ...
  const recentScoreboards = scoreboards.slice(-recentWeeks);
